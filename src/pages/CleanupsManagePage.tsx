...
function ManageInner() {
  ...
  if (!profile?.is_admin) {
    return (
      <p className="rounded-[var(--radius-card)] border border-amber-400/30 p-4 text-sm text-amber-100">
  ...
  return (
    <div className="space-y-8">
      {status && <p className="text-sm text-gold-200">{status}</p>}
...
export function CleanupsManagePage() {
  return (
    <div className="fn-rebuild min-h-full bg-[#f3f7f4] text-[#12332d]">
      <SiteNav />
      <main className="mx-auto w-full max-w-7xl px-5 pb-20 pt-28 sm:px-8 lg:px-12">
