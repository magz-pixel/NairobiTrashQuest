...
export function PageIntro({ eyebrow, title, body, action }: { eyebrow: string; title: string; body?: string; action?: ReactNode }) {
  return <header className="fn-intro"><div className="fn-intro-index" aria-hidden="true"><span>FN</span><b>2026</b></div><div className="fn-intro-copy"><p className="fn-eyebrow">{eyebrow}</p><h1 className="fn-display">{title}</h1>{body && <p className="fn-intro-body">{body}</p>}{action && <div className="fn-intro-actions">{action}</div>}</div><div className="fn-intro-signal" aria-hidden="true"><span>FIELD SIGNAL</span><i /><i /><i /><i /></div></header>
}
...
export function ActionLink({ to, children, tone = 'dark' }: { to: string; children: ReactNode; tone?: 'dark' | 'gold' | 'light' }) {
  ...
  return <Link to={to} className={cls}><span>{children}</span><b aria-hidden>↗</b></Link>
}
...
export function OpsFrame({ title, eyebrow, description, backTo, children }: { title: string; eyebrow: string; description: string; backTo?: string; children: ReactNode }) {
  return <PublicShell><main className="fn-ops-page"><div className="fn-ops-wrap">{backTo && <Link to={backTo} className="fn-back">← Back to public view</Link>}<div className="fn-ops-heading"><SectionLabel>{eyebrow}</SectionLabel><h1 className="fn-display">{title}</h1><p>{description}</p></div><div className="mt-10">{children}</div></div></main></PublicShell>
}
