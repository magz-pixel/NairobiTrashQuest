...
export function ProfileBadge({ className = '' }: { className?: string }) {
  ...
  return (
    <div
      className={`flex w-full min-w-0 max-w-full items-center gap-2 rounded-[var(--radius-card)] border border-[var(--border-subtle)] bg-[var(--bg-surface)] px-2.5 py-2 text-sm shadow-[var(--shadow-sm)] sm:max-w-[13.5rem] sm:gap-3 sm:px-3 ${className}`}
    >
