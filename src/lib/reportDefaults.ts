...
export function demoReportDefaults(
  partial: Omit<
    ...
    | 'is_anonymous'
  > &
    Partial<Pick<Report, 'waste_type' | 'seen_count' | 'area_name' | 'ward_id'>>,
): Report {
  const ward = assignWard(partial.latitude, partial.longitude)
  return {
    waste_type: partial.waste_type ?? WASTE_TYPES[Math.floor(Math.random() * WASTE_TYPES.length)],
    seen_count: partial.seen_count ?? Math.floor(Math.random() * 12),
    flag_count: 0,
    approved_at: partial.created_at,
